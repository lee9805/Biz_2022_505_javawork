...
public class ArraysV3 {
   ...
   public static void main(String[] args) {
      ...
      // index라는 변수를 사용하여 첨자를 지정하므로써 반복되는 코드가 만들어지고 단순한 코드가 생성되었다.
      ...
      // index 값을 0부터 1씩 증가시키면서 5미만인 동안 {}의 코드를 실행한다.
      ...
      /*
       * 위 코드에서 데이터의 개수를 200으로 변경(new int[200]) 했을 때 for() 명령문의 index<100dmf index<200으로 변경해 주어야한다.
       * 만약 for()명령문의 코드를 변경하지 않으면 테스트 데이터의 개수를 늘린 효과가 없어진다.
       * 
       * 배열의 개수를 변경할 때마다 코드를 추가로 변경하는 불편을 없애기 위해 자바에서는 배열의 크기와 관련된 내부 속성을 마련해 두었다.
       * 배열.length 속성은 배열의 개수를 담고 있는 특별한 변수이다.
       * 배열.length는 배열을 생성하면 배열의 개수를 보관하도록 자동으로 만들어지는 특별할 변수이다.
