...
public class ScoreServiceImplV3 extends ScoreServiceImplV2{
	...
	public void saveScore() {
		 ...
		 * 실제 데이터를 기록하기 위해서는 FileOutputStream 만
		 * 있어도 된다
		 * 하지만, 파일에 기록하고 저장하는 일은 상당히 많은 시간을 소모한다
		 * 컴퓨터 메모리, CPU 입장에서는 파일을 기록하는 동안
		 * 아무것도 하지 못하고 기다려야 하는 상황이 발생한다
		 * 
		 * 이때 중간 BuffereOutput 라는 파이프를 연결해 두고
		 * 애플리케이션에서는  에게 데이터를 모두 보내고
		 * 다른 일을 수행한다
		 * 
		 * 그러면  bufferedOutput 와 가 협력하여 데이터를 
