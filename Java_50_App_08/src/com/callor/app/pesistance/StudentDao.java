...
public class StudentDao {
	 ...
	 * 1. Excetion 이 발생할 만한 코드를 try-catch 로 묶어 주는 방법 
	 * 2. Excetion 을 나를 호출한 곳으로 던지기
	 * 
	 * Service에서 다음과 같이 selectAll() 을 호출학 것이다 
	 * List<StudentVO> stList =dao.selectAll(); 
	 * dao.selectAll()에서 Excetion이 발생하면 
	 * dao.selectAll() 에서 Excetion을 직접 처리(핸들링)하지 않고 
	 * Service로 Excetion 던지기(전달)하여 을 모아서 처리하기
	 */
	...
	private List<StudentVO> getResult(ResultSet rSet) throws SQLException {
		...
		while (rSet.next()) {
			StudentVO stVO = StudentVO.builder().stNum(rSet.getString(DBContract.ST_COL.ST_NUM))
					...
					.stTel(rSet.getString(DBContract.ST_COL.ST_TEL)).build();
			stList.add(stVO);
		}
		return stList;
	}
