...
public class VarEx3 {
	...
	public static void main(String[] args) {
		 ...
		 * 하지만 그렇게 하지 않는다
		 * 왜, 메모리 관리, 유효성(오류를 날수 있는 코드)관리 등이 어렵기 때문
		 */
		List<Object> objlist;
		
		List<Integer> intList;
		//데이터의 유효성 검사
		//Integer type 만 add 할 수 있는 intList 에
		//Float type 의 값을 추가하려고 하니 오류가 발생
		//intList.add(30.0f);
		
		/*
		 * stList 에는 StudentVO type 의 데이터만 추가할 수있다
		 * 만약 다른 type 의 데이터가 추가되면 오류가 난다
		 * 
		 * 이러한 기능을 사용하지 않으면
		 * stList 에 저장된 데이터가 유효한지 잘못되었는지
		 * 항상 각종하면서 코딩을 해야 할 것이다
		 * 
		 * 사전에 제네릭을 선언 하므로써
		 * 개발자는 데이터에 대한 유효성 문제를 걱정하지 않아도 된다
		 * 
		 * 즉, stList 에 담긴 학생 정보들은 "무결성"을 유지 한다
		 * 
		 */
